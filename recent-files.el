...
;;; $Header: recent-files.el[1.9] Wed Jan 18 21:42:36 1995 nickel@prz.tu-berlin.de saved $
;;;
...
Entries are pairs like (<filename> . <permant-p>).
...
If called interactively, show it in the echo area."
  (interactive)
  (let ((version "$Header: recent-files.el[1.9] Wed Jan 18 21:42:36 1995 nickel@prz.tu-berlin.de saved $"))
    (if (interactive-p)
        (message version)
      version)))
  ...
  (if (or recent-files-list-changed-p
      ...
      (let ((action-menu-entries
        ...
        (if (or recent-files-permanent-submenu
            ...
            (progn
              (setq permanent (recent-files-make-file-menu-entries
                               recent-files-list
                               (function recent-files-filter-permanent)))
              (setq non-permanent (recent-files-make-file-menu-entries
                                   recent-files-list
                                   (function
                                    recent-files-filter-non-permanent))))
          (setq all (recent-files-make-file-menu-entries
                     recent-files-list
                     (function (lambda (l) l)))))
        (if recent-files-permanent-submenu
            (setq permanent (list (cons "Permanent entries..." permanent))))
        (if recent-files-non-permanent-submenu
            (setq non-permanent (list (cons "Non-permanent entries..."
                                            non-permanent))))
    ;;; ... and now even uglier.
...
in recent-files-filename-replacements with the corrensponding replacement.
...
`recent-files-dont-include'."
  (if (recent-files-no-match filename recent-files-dont-include)
      (progn
        (setq recent-files-list-changed-p t)
